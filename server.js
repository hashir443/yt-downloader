...
// GET /download — YouTube: writes to temp file then streams
...
// GET /social-video-download — Instagram & Facebook: same temp-file pipeline as YouTube.
// Guarantees Content-Disposition: attachment so browser saves instead of playing inline.
app.get('/social-video-download', (req, res) => {
  const url = (req.query.url || '').trim();
  if (!url)        return res.status(400).json({ error: 'url required' });
  if (!FFMPEG_BIN) return res.status(500).json({ error: 'ffmpeg not installed' });
  ...
  console.log('[social-video-download] url=%s', url.slice(0, 80));

  execFile(YTDLP, args, { timeout: 120_000, maxBuffer: 50 * 1024 * 1024 }, (err, _stdout, stderr) => {
    if (err) {
      const ytErr = (stderr || '').split('\n')
        .find(l => l.includes('ERROR:'))?.replace(/.*ERROR:\s*/, '').trim()
        || (stderr || '').slice(-500)
        || err.message;
      console.error('[social-video-download] error:', ytErr);
      return res.status(500).json({ error: ytErr });
    }
    ...
    const outFile = files[0];
    if (!outFile || !fs.existsSync(outFile)) {
      console.error('[social-video-download] file missing. stderr:', stderr.slice(-300));
    ...
    console.log('[social-video-download] streaming %s MB as .%s', (size / 1e6).toFixed(1), ext);
    ...
    stream.on('close', () => fs.unlink(outFile, () => {}));
    stream.on('error', (e) => {
      console.error('[social-video-download] stream error:', e.message);
      fs.unlink(outFile, () => {});
    });
  });
});

// GET /direct-url — resolves CDN URL(s) via yt-dlp -g
...
// GET /social-download — CDN proxy fallback (kept for compatibility)
